...
class FactoringRequest(BaseModel):
    ...
    def annualized_cost(self):
        ...
        if self.invoices.exists():
            ...
            avg_days = total_days / self.invoices.count() if self.invoices.count() > 0 else 30
        
        if avg_days <= 0:
            return 0.0
        
        cost_rate = (self.factoring_fee_amount / self.advance_amount) if self.advance_amount > 0 else 0
