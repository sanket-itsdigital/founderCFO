...
class WriteOffsSummaryView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        candidates = (
            Invoice.objects.filter(company=company, due_date__lt=cutoff_date)
            .exclude(
                status__in=[InvoicesStatusChoices.PAID, InvoicesStatusChoices.CANCELLED]
            )
            .filter(total_amount__gt=F("paid_amount"))
            .exclude(write_offs__isnull=False)  # Exclude already written off
        )
        ...
        written_off_amount = written_off.aggregate(total=Sum("write_off_amount"))[
            "total"
        ] or Decimal("0.00")
        ...
        return Response(
            {
                ...
                "already_written_off_amount_display": self._in_lakhs(
                    written_off_amount
                ),
            }
        )
...
class WriteOffCandidatesListView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        candidates = (
            ...
            .filter(total_amount__gt=F("paid_amount"))
            .exclude(write_offs__isnull=False)
        )
...
class WriteOffListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            WriteOff.objects.filter(company=company)
    ...
    def perform_create(self, serializer):
        ...
        write_off = serializer.save(company=company, write_off_amount=write_off_amount)
...
class WriteOffSelectedView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        invoices = (
            Invoice.objects.filter(id__in=invoice_ids, company=company)
            .exclude(
                status__in=[InvoicesStatusChoices.PAID, InvoicesStatusChoices.CANCELLED]
            )
            .exclude(write_offs__isnull=False)
