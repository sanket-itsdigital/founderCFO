...
class CollectionPriorityView(APIView):
    ...
    def _calculate_priority(days_overdue, outstanding_amount, risk_level):
        ...
        if days_overdue >= 90 or (days_overdue >= 60 and risk_level == RiskLevelChoices.HIGH) or \
           (outstanding_amount >= Decimal("5000000") and days_overdue >= 30):
            return "Critical", "#EF4444"  # Red
        
        # High Priority: 60+ days overdue OR (30+ days AND medium/high risk) OR (outstanding > 20L AND 15+ days)
        elif days_overdue >= 60 or (days_overdue >= 30 and risk_level in [RiskLevelChoices.MEDIUM, RiskLevelChoices.HIGH]) or \
             (outstanding_amount >= Decimal("2000000") and days_overdue >= 15):
    ...
    def get(self, request, *args, **kwargs):
        ...
        company = get_company_from_request(request)
        credits = {credit.customer_name: credit for credit in Credit.objects.filter(company=company)}
        ...
        for invoice in invoices:
            ...
            if customer_data[customer_name]["oldest_due_date"] is None or \
               invoice.due_date < customer_data[customer_name]["oldest_due_date"]:
        ...
        for customer_name, data in customer_data.items():
            ...
            days_overdue = (today - data["oldest_due_date"]).days if data["oldest_due_date"] else 0
            ...
            expected_recovery = data["outstanding"] * Decimal(str(recovery_percentage / 100))
        ...
        customers_list.sort(key=lambda x: (priority_order.get(x["priority"], 99), -x["days_overdue"]))
